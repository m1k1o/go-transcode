...
if echo "$INIT_HW_DEVICE" | grep "cuda" > /dev/null; then
    echo "[OK] CUDA is supported by ffmpeg."
else
    echo "[ERR] CUDA is not supported by ffmpeg."
    CUDA_SUPPORTED="no"
fi
...
if echo "$INIT_HW_DEVICE" | grep "vaapi" > /dev/null; then
    echo "[OK] VAAPI is supported by ffmpeg."
else
    echo "[ERR] VAAPI is not supported by ffmpeg."
    VAAPI_SUPPORTED="no"
fi

if vainfo 2>&1 > /dev/null; then
    echo "[OK] vainfo command executed succesfully."
else
    echo "[ERR] vainfo command not found."
    VAAPI_SUPPORTED="no"
fi

DRI_RENDER="/dev/dri/renderD128"
if [ -e $DRI_RENDER ]; then
    echo [OK] "$DRI_RENDER exists."
else
    echo "[ERR] $DRI_RENDER not found."
    VAAPI_SUPPORTED="no"
fi
...
if [ "$CUDA_SUPPORTED" = "yes" ]; then
    ...
    if [ "$?" = "1" ]; then
        echo "[ERR] Unsupported codec: ${ENCODER:-???}"
    elif [[ ! " ${AVAILABLE_ENCODERS} " =~ " ${ENCODER} " ]]; then
        echo "[ERR] Unsupported encoder: ${ENCODER:-???}"
    else
        echo "Using CUDA hardware, codec $ENCODER."
    ...
    if [ "$?" = "1" ]; then
        ...
        echo "Using using VAAPI hardware, codec $ENCODER."
